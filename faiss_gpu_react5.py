...
class CMSDenialAnalyzer:
    ...
    def analyze_claim(self, claim_data):
        ...
        # In the analyze_claim method, modify the retrieval logging section:
        ...
        for rank, idx in enumerate(I[0]):
            ...
            # Handle query embedding conversion
            if isinstance(query_embedding, np.ndarray):
                query_embed_short = query_embedding[:10].tolist()
            else:
                query_embed_short = query_embedding[:10]  # Assume it's already a list
            
            # Handle chunk embedding conversion
            if isinstance(chunk_embedding, np.ndarray):
                chunk_embed_short = chunk_embedding[:10].tolist()
            else:
                chunk_embed_short = chunk_embedding[:10]  # Assume it's already a list
            
            log_entry = {
                "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat(),  # Fixed UTC deprecation
                "query": query,
                "query_embedding": query_embed_short,
                "chunk_index": int(idx),
                "chunk_embedding": chunk_embed_short,
    ...
    def _generate_query(self, claim):
        ...
        # Add this to your CMSDenialAnalyzer class
        
    def save_visualization_data(self, claim_data, output_path="visualization_data.json"):
        result = self.analyze_claim(claim_data)

        # Re-run retrieval to get full documents and metadata
        retrieved_docs = self.retrieval["retriever"].invoke(claim_data["cpt_code"])

        # Filter docs using same logic as analyze_claim
        filtered_docs = [
            doc for doc in retrieved_docs if "modifier" in doc.page_content.lower()
            or "billing" in doc.page_content.lower() or "e/m" in doc.page_content.lower()
        ]
        ...
        for i, doc in enumerate(filtered_docs[:5]):  # Limit to top 5 for clarity
            doc_id = f"doc-{i}"
            similarity = result.get("scores", [])[i] if i < len(result.get("scores", [])) else 0.0
            ...
            visualization_data["documents"].append({
                "id": doc_id,
