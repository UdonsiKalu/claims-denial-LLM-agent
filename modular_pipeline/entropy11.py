...
if "entropy_df" in st.session_state:
    ...
    with col2:
        ...
        matrix_long_df = matrix_df.reset_index().melt(id_vars="index", var_name="Chunk", value_name="Retrieval Count")
        matrix_long_df = matrix_long_df.rename(columns={"index": "Query"})
        matrix_long_df = matrix_long_df[matrix_long_df["Retrieval Count"] > 0]

        fig = px.density_heatmap(
            data_frame=matrix_long_df,
            x="Chunk",
            y="Query",
            z="Retrieval Count",
            color_continuous_scale="YlGnBu",
            title="Entropy Map: Query vs Chunk Retrievals",
            height=600,
        )
        fig.update_layout(
            xaxis_title="Chunks",
