...
# === SESSION STATE FOR CLICKABLE BINS ===
...
with tab1:
    ...
    if "entropy_df" in st.session_state:
        ...
        with col2:
            ...
            # Extract per-row spectral features
            ...
            # --- Build DataFrames for FFT bin activity ---
            df_chunks_bins = pd.DataFrame({
                "bin_idx": np.arange(len(freqs_c)),
                "perc_active_chunks": np.mean(P_chunks > 0, axis=0) * 100
            })

            df_queries_bins = pd.DataFrame({
                "bin_idx": np.arange(len(freqs_q)),
                "perc_active_queries": np.mean(P_queries > 0, axis=0) * 100
            })

            # --- Plotly histogram for queries ---
            fig_queries = px.bar(
                df_queries_bins,
                x="bin_idx",
                y="perc_active_queries",
                title="Bin activity across queries (clickable)",
                labels={"perc_active_queries": "% rows active"}
            )
            selected_query_points = plotly_events(fig_queries, click_event=True, hover_event=False)
            st.plotly_chart(fig_queries, use_container_width=True)

            # --- Plotly histogram for chunks ---
            fig_chunks = px.bar(
                df_chunks_bins,
                x="bin_idx",
                y="perc_active_chunks",
                title="Bin activity across chunks (clickable)",
                labels={"perc_active_chunks": "% rows active"}
            )
            selected_chunk_points = plotly_events(fig_chunks, click_event=True, hover_event=False)
            st.plotly_chart(fig_chunks, use_container_width=True)

            # --- Sync slider with click events ---
            if selected_chunk_points:
                st.session_state.selected_bin = int(selected_chunk_points[0]["x"])
            elif selected_query_points:
                st.session_state.selected_bin = int(selected_query_points[0]["x"])

            # --- FFT bin selector (linked to state) ---
            bin_idx = st.slider(
                "FFT Bin Index",
                min_value=1,
                max_value=len(freqs_q)-1,
                value=st.session_state.get("selected_bin", 10),
                step=1
            )
            ...
            if selected_chunk_points:
                clicked_bin = int(selected_chunk_points[0]["x"])
                st.session_state.selected_bin = clicked_bin
                st.success(f"Updated selected bin → {clicked_bin}")
            ...
            if len(feat_chunks) > 0:
                st.write("Example chunk feature:", feat_chunks[0])

            # Collect peak indices and PNRs
            ...
            bin_idx = st.slider(
                ...
                min_value=1,
                max_value=len(freqs_q)-1,
                value=st.session_state.selected_bin or 10,
                step=1
            )
            ...
            # Auto-select the most common peak frequency
            if np.any(peak_idx_chunks > 0):
            ...
            fft_results = pd.DataFrame({
                "chunk_id": matrix_df.columns,
                "peak_idx": peak_idx_chunks,
                "pnr_db": pnr_chunks
            })
            st.download_button(
