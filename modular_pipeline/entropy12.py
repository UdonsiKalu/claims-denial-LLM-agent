    ...
    with col2:
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                title="Chunks",
                tickmode='linear',
                dtick=max(1, len(matrix_display_df.columns) // 10),  # show every ~10%
                tickangle=45,
            ),
            yaxis=dict(
                title="Queries",
                tickmode='linear',
                dtick=max(1, len(matrix_display_df.index) // 10),  # show every ~10%
            ),
            margin=dict(l=80, r=50, t=50, b=100),
        )


        # Optional: Enable tick labels if matrix is small
