    ...
    if st.button("🚀 Run Optimizer") and claims:
        ...
        with st.spinner("Evaluating strategy combinations..."):
            ...
            if search_method == "Grid Search":
                ...
                #full_history = bayesian_search(param_grid, lambda p: evaluate_strategy(p, claims, target_profile), n_calls=n_calls, log_fn=append_log)
            ...
            for i, entry in enumerate(sorted(full_history, key=lambda x: x['score'], reverse=True)[:5]):
                ...
                # 🔧 Convert np.int64 and np.float64 to native Python types
                safe_entry = {
                    k: (
                ...
                st.code(json.dumps(safe_entry, indent=2))  # ✅ Use the fixed dict
