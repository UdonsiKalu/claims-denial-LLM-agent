...
if mode == "Optimizer":
    ...
    if st.button("🚀 Run Optimizer") and claims:
        ...
        with st.spinner("Evaluating strategy combinations..."):
            ...
            def evaluate_strategy(params, claims=claims, target_profile=target_profile, idx=None, total=None):
                ...
                return analyzer.evaluate_entropy_score(claims, target_profile, log_fn=append_log)
            ...
            if search_method == "Grid Search":
                full_history = grid_search(param_grid, lambda p: evaluate_strategy(p))
            elif search_method == "Random Search":
                full_history = random_search(param_grid, lambda p: evaluate_strategy(p), n_iter=n_iter)
            elif search_method == "Bayesian Optimization":
                full_history = bayesian_search(param_grid, lambda p: evaluate_strategy(p), n_calls=n_calls)
