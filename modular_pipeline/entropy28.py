...
def plot_optimizer_results(df):
    ...
    if "entropy" in df.columns and "coverage" in df.columns:
...
if mode == "Optimizer":
    ...
    if st.button("🚀 Run Optimizer") and claims:
        ...
        with st.spinner("Evaluating strategy combinations..."):
            ...
            # 🔁 Define evaluate_strategy inside spinner or above
            def evaluate_strategy(params, claims, target_profile, idx=None, total=None):
                ...
                result = CMSDenialAnalyzer(
                    exclude_tokens=[],
                    faiss_k=params["faiss_k"],
                    bm25_k=params["bm25_k"],
                    faiss_fetch_k=params["fetch_k"],
                    weights=params["weights"]
                ).evaluate_entropy_score(claims, target_profile, log_fn=append_log)
            ...
            def apply_best_config_to_session(config):
                st.session_state["frozen_params"] = {
                    "chunking_strategy": "Fixed-size",  # or infer from session or set dynamically
                    "chunk_size": 10000,
                    "chunk_overlap": 2000,
            ...
            param_grid = {
                ...
                "fetch_k": [25, 50],  # Make sure 'fetch_k' is present
                "weights": [(0.4, 0.6), (0.5, 0.5), (0.6, 0.4)]
            }

            # Run selected search
            if search_method == "Grid Search":
                full_history = grid_search(param_grid, lambda p: evaluate_strategy(p, claims, target_profile))
            elif search_method == "Random Search":
                full_history = random_search(param_grid, lambda p: evaluate_strategy(p, claims, target_profile), n_iter=n_iter)

            elif search_method == "Bayesian Optimization":
                full_history = bayesian_search(param_grid, lambda p: evaluate_strategy(p, claims, target_profile), n_calls=n_calls)
