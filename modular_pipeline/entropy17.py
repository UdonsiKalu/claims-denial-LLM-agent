...
# === INITIALIZATION PARAM TRACKING ===
# Save the first-time loaded settings so we don't reload when sliders change
...
if not st.session_state["retriever_initialized"]:
    with st.spinner("⚙️ Initializing retrieval system with default settings..."):
        params = st.session_state["retriever_settings"]
        analyzer = load_analyzer(
            params["chunking_strategy"],
            params["chunk_size"],
            params["chunk_overlap"],
            params["exclude_tokens"],
            False,
            params["faiss_k"],
            params["bm25_k"],
            params["faiss_fetch_k"],
            params["weights"]
        )
