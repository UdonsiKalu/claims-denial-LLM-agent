    ...
    if st.button("🚀 Run Optimizer") and claims:
        ...
        with st.spinner("Evaluating strategy combinations..."):
            ...
            def evaluate_strategy(params, claims, target_profile, idx=None, total=None):
                ...
                # Create the strategy_id dynamically from the params
                strategy_id = f"faiss{params['faiss_k']}_bm25{params['bm25_k']}_fetch{params['fetch_k']}_w{params['weights']}"
                params["strategy_id"] = strategy_id  # Add the strategy_id to the params dictionary

                # Apply the filtering logic based on excluded tokens
                exclude_tokens = st.session_state.get("exclude_tokens_runtime", [])
                filtered_claims = filter_chunks(claims, exclude_tokens)  # Filter claims based on exclude tokens

                # Use the filtered claims for analysis
                analyzer = CMSDenialAnalyzer(
                    exclude_tokens=[],  # We're passing only included chunks here
                ...
                result = analyzer.evaluate_entropy_score(
                    filtered_claims,  # Now passing filtered claims (only included chunks)
                    target_profile,
                    log_fn=append_log,
                    idx=idx,
                    total=total
                ...
                # Log the penalty breakdown and final score for this strategy
                append_log(f"Strategy: {params['strategy_id']}")
                append_log(f"  Entropy Penalty: {result['penalties']['entropy_penalty']}")
                append_log(f"  Frequency Penalty: {result['penalties']['frequency_penalty']}")
                append_log(f"  Gini Penalty: {result['penalties']['gini_penalty']}")
                append_log(f"  Coverage Penalty: {result['penalties']['coverage_penalty']}")
                append_log(f"Final Score: {result['score']}")
