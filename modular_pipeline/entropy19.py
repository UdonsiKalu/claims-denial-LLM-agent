# ✅ REVISED: Retrieval Studio with Frozen Params and Button-Gated Reloading
...
ui_chunk_size = st.sidebar.slider("Chunk Size", 1000, 20000, 10000, 500)
ui_chunk_overlap = st.sidebar.slider("Chunk Overlap", 0, 5000, 2000, 200)

st.sidebar.header("Retriever Settings")
ui_faiss_k = st.sidebar.slider("FAISS Top-k", 1, 20, 5)
ui_bm25_k = st.sidebar.slider("BM25 Top-k", 1, 20, 3)
ui_faiss_fetch_k = st.sidebar.slider("FAISS Fetch_k (MMR)", ui_faiss_k, 100, 50)
ui_faiss_weight = st.sidebar.slider("Weight: FAISS", 0.0, 1.0, 0.6, 0.05)
...
if st.sidebar.button("\Rechunk & Reindex"):
    st.session_state["frozen_params"] = {
        ...
        "weights": ui_weights,
        "exclude_tokens": ui_exclude_tokens
    }
