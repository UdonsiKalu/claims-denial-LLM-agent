...
# === FREEZE SETTINGS FOR FIRST LOAD ===
if not st.session_state["retriever_initialized"]:
    # Freeze default settings so future reruns don't trigger analyzer reload
    st.session_state["frozen_chunking_strategy"] = st.session_state.get("frozen_chunking_strategy", "Fixed-size")
    st.session_state["frozen_chunk_size"] = st.session_state.get("frozen_chunk_size", 10000)
    st.session_state["frozen_chunk_overlap"] = st.session_state.get("frozen_chunk_overlap", 2000)
    st.session_state["frozen_faiss_k"] = st.session_state.get("frozen_faiss_k", 5)
    st.session_state["frozen_bm25_k"] = st.session_state.get("frozen_bm25_k", 3)
    st.session_state["frozen_faiss_fetch_k"] = st.session_state.get("frozen_faiss_fetch_k", 50)
    st.session_state["frozen_weights"] = st.session_state.get("frozen_weights", (0.4, 0.6))
    st.session_state["frozen_exclude_tokens"] = st.session_state.get("frozen_exclude_tokens", ["page_0", "introduction", "scope", "purpose"])
...
if not st.session_state["retriever_initialized"]:
    with st.spinner("⚙️ Initializing retrieval system with default settings..."):
        analyzer = load_analyzer(
            st.session_state["frozen_chunking_strategy"],
            st.session_state["frozen_chunk_size"],
            st.session_state["frozen_chunk_overlap"],
            st.session_state["frozen_exclude_tokens"],
            False,
            st.session_state["frozen_faiss_k"],
            st.session_state["frozen_bm25_k"],
            st.session_state["frozen_faiss_fetch_k"],
            st.session_state["frozen_weights"]
        )
        st.session_state["retriever"] = analyzer.retrieval["retriever"]
        st.session_state["analyzer"] = analyzer
        st.session_state["retriever_initialized"] = True
        st.success("Retriever initialized with default settings.")

# === REBUILD HANDLER (on button click only) ===
elif st.session_state["rebuild_index"]:
    with st.spinner("🔁 Rebuilding retrieval system..."):
        ...
        st.success("Reindexed with new settings.")
