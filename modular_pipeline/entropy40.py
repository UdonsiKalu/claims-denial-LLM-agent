...
if st.sidebar.checkbox("Force Rebuild Vector DB", key="force_rebuild"):
    coll = os.environ.get("QDRANT_COLLECTION", "cms_chunks")
    client = QdrantClient(host="localhost", port=6333)
    try:
        client.delete_collection(coll)
        st.warning(f"Deleted '{coll}'. Will rebuild on next run.")
    except Exception as e:
        st.error(f"Could not delete collection '{coll}': {e}")
...
def load_analyzer(params, force_rebuild=False, chunks_key=None):
    # Get actual cleaned chunks from session state using the key
    chunks_override = st.session_state.get("chunks_override") if chunks_key else None
    ...
    # Use a unique collection name for cleaned chunks
    coll_suffix = f"_clean{len(chunks_override)}" if chunks_override else ""
    os.environ["QDRANT_COLLECTION"] = (
        f"qdrant_{params['chunking_strategy']}_c{params['chunk_size']}_o{params['chunk_overlap']}{coll_suffix}"
    )
    ...
    return CMSDenialAnalyzer(
        ...
        preloaded_chunks=chunks_override
    )
...
if not st.session_state["retriever_initialized"] or st.session_state["rebuild_index"]:
    with st.spinner("Initializing or Rebuilding Retriever..."):
        ...
        clean = st.session_state.get("chunks_override")  # <- get cleaned chunks (may be None)

        analyzer = load_analyzer(
            frozen_params,
            force_rebuild=st.session_state.get("rebuild_index", False),
            chunks_key=(len(clean) if clean else None)
        )
        st.session_state["retriever"] = analyzer.retrieval["retriever"]
        st.session_state["analyzer"] = analyzer
...
###########################################
with st.sidebar.expander("🧹 Remove chunks (rebuild index)"):
    analyzer = st.session_state.get("analyzer")
    if analyzer:
        # Build a list of chunk IDs once
        if "all_chunk_ids" not in st.session_state:
            ids = []
            for d in analyzer.chunks:
                cid = d.metadata.get("chunk_id") or f"{d.metadata.get('source','unknown')}::Page_{d.metadata.get('page',0)}"
                ids.append(cid)
            st.session_state["all_chunk_ids"] = ids

        selected_to_remove = st.multiselect(
            "Select chunk IDs to remove",
            options=st.session_state["all_chunk_ids"],
            default=[],
            max_selections=500
        )

        if st.button("Apply removal & rebuild"):
            # Map current chunks to IDs
            def _cid(doc):
                return doc.metadata.get("chunk_id") or f"{doc.metadata.get('source','unknown')}::Page_{doc.metadata.get('page',0)}"

            cleaned_chunks = [d for d in analyzer.chunks if _cid(d) not in selected_to_remove]

            # ✅ Save cleaned set and trigger rebuild
            st.session_state["chunks_override"] = cleaned_chunks
            st.session_state["rebuild_index"] = True

            # Clear cached resource so load_analyzer runs fresh
            try:
                load_analyzer.clear()
            except Exception:
                pass

            st.success(f"Removed {len(analyzer.chunks) - len(cleaned_chunks)} chunks. Rebuilding…")
            st.rerun()
    else:
        st.info("Initialize the retriever first to load chunks.")
...
with tab1:
    ...
    if claims_file:
        try:
            claims = [json.loads(line) for line in claims_file]
            st.session_state["claims"] = claims  # ✅ store in session state
            st.success(f"Loaded {len(claims)} claims")
        except Exception as e:
        ...
        # ✅ always available before you press the button
