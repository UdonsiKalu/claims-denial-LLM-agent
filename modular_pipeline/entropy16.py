...
if st.session_state["rebuild_index"]:
    with st.spinner("🔁 Rebuilding retrieval system..."):
        analyzer = load_analyzer(
            chunking_strategy, chunk_size, chunk_overlap, exclude_tokens, True,
            faiss_k, bm25_k, faiss_fetch_k, weights
        )
...
# 🟢 First-time load only (default settings)
elif not st.session_state["retriever_initialized"]:
    with st.spinner("⚙️ Initializing retrieval system with default settings..."):
        analyzer = load_analyzer(
            chunking_strategy, chunk_size, chunk_overlap, exclude_tokens, False,
            faiss_k, bm25_k, faiss_fetch_k, weights
        )
        st.session_state["retriever"] = analyzer.retrieval["retriever"]
        st.session_state["analyzer"] = analyzer
