...
st.sidebar.markdown("### 🧠 Frequency Map Options")
filter_to_entropy_chunks = st.sidebar.checkbox("Show only tokens in top retrieved chunks", value=False)
...
if "token_freq_df" in st.session_state and not st.session_state["token_freq_df"].empty:
    ...
    if filter_to_entropy_chunks and "entropy_df" in st.session_state:
        top_chunk_ids = set(st.session_state["entropy_df"].head(20)["chunk_id"])
        freq_df = freq_df[
            freq_df.apply(lambda row: f"{row['source']}::Page_{row['page']}" in top_chunk_ids, axis=1)
        ]
