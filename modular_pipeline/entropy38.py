...
# --- Rebuild/apply helpers (top-level, survive reruns) ---
def apply_best_config_to_session(config):
    st.session_state["frozen_params"] = {
        "chunking_strategy": "Fixed-size",  # keep as-is unless you add strategy search
        "chunk_size": config.get("chunk_size", 10000),
        "chunk_overlap": config.get("chunk_overlap", 2000),
        "faiss_k": int(config["faiss_k"]),
        "bm25_k": int(config["bm25_k"]),
        "faiss_fetch_k": int(config["fetch_k"]),
        "weights": (
            tuple(map(float, config["weights"].split(",")))
            if isinstance(config.get("weights"), str)
            else config["weights"]
        ),
        "exclude_tokens": st.session_state.get("exclude_tokens_runtime", []),
    }
    st.session_state["rebuild_index"] = True  # important: signal a real rebuild

def recompute_entropy_map(retriever, claims):
    log = defaultdict(int)
    for claim in claims:
        try:
    ...
    df = pd.DataFrame(
        [{"chunk_id": k, "retrieval_count": v} for k, v in log.items()]
...
if mode == "Optimizer":
    ...
    if claims_file:
        try:
            ...
            st.session_state["last_uploaded_claims"] = claims 
        except Exception as e:
    ...
    if st.button("🚀 Run Optimizer") and claims:
        ...
        exclude_tokens_rt = [t.lower() for t in st.session_state.get("exclude_tokens_runtime", [])]

        if any("chunks" in c for c in claims):
            total_chunks = sum(len(claim.get("chunks", [])) for claim in claims)
            excluded_chunks = sum(
                any(tok in (chunk or "").lower() for tok in exclude_tokens_rt)
                for claim in claims
                for chunk in claim.get("chunks", [])
            )
            st.write(f"Excluding {excluded_chunks} of {total_chunks} chunks based on the provided tokens.")
        else:
            st.info("No 'chunks' arrays found in claims; skipping exclude summary.")
...
###########################################################################
            if full_history:
                ...
                st.session_state["best_config"] = best  # <-- put it here

                # (optional) also keep history
                st.session_state["optimizer_history"] = full_history
...
###############################################################################
            ...
            if st.button("📥 Apply Best Config and Rebuild Index"):
                ...
                ############
                # After index rebuild with best configuration
                claims_cached = st.session_state.get("last_uploaded_claims")
                if claims_cached:
                    with st.spinner("Recomputing entropy map on the new index..."):
                        new_df = recompute_entropy_map(st.session_state["retriever"], claims_cached)
                        st.session_state["entropy_df"] = new_df
                        st.session_state["last_run"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    st.success("✅ Recomputed entropy map using optimized settings.")
                else:
                    st.info("ℹ️ Rebuilt index. Upload claims and click ‘Generate Entropy Map’ to see changes.")
        ...
        ###################################################
        st.markdown("---")
        st.subheader("Apply Optimized Configuration")

        best_cfg = st.session_state.get("best_config")
        if best_cfg:
            st.json(best_cfg, expanded=False)

        if st.button("📥 Apply Best Config and Rebuild Index", key="apply_best_global"):
            if not best_cfg:
                st.error("No best config found. Run the optimizer first.")
            else:
                # 1) Save best into frozen params
                apply_best_config_to_session(best_cfg)

                # 2) Force a true rebuild
                bp = st.session_state["frozen_params"]
                os.environ["CHUNK_STRATEGY"] = bp["chunking_strategy"]
                os.environ["CHUNK_SIZE"] = str(bp["chunk_size"])
                os.environ["CHUNK_OVERLAP"] = str(bp["chunk_overlap"])
                os.environ["QDRANT_COLLECTION"] = f"qdrant_{bp['chunking_strategy']}_c{bp['chunk_size']}_o{bp['chunk_overlap']}"
                os.environ["FORCE_REBUILD"] = "1"
                ...
                with st.spinner("Rebuilding index with best configuration..."):
                    analyzer = load_analyzer(bp, force_rebuild=True)
                    st.session_state["retriever"] = analyzer.retrieval["retriever"]
                    st.session_state["analyzer"]  = analyzer
                    st.success("✅ Index rebuilt with best configuration.")

                # 3) Auto-refresh Entropy Map if we have claims cached
                claims_cached = st.session_state.get("last_uploaded_claims")
                if claims_cached:
                    with st.spinner("Recomputing entropy map on the new index..."):
                        new_df = recompute_entropy_map(st.session_state["retriever"], claims_cached)
